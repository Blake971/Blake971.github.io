...
const DEFAULT_SETTINGS: SiteSettings = {
  ...
  contactEmail: "hello@visezworks.com",
...
export function normalizeProjectCategory(cat: string): ProjectCategory {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((p) => ({
          ...
          category: normalizeProjectCategory(String(p.category || "")),
...
const EVENT_NAME = "visezworks_admin_store_change";
