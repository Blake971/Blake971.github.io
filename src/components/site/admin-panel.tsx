...
export function AdminPanel() {
  ...
  const handleSaveProject = (e: React.FormEvent) => {
    ...
    if (isAddingProject) {
      const newProj: ProjectItem = {
        ...
        category: (editingProject.category as ProjectCategory) || "Web Development",
        image: editingProject.image || "",
        videoUrl: editingProject.videoUrl || "",
        link: editingProject.link || "#",
        featured: editingProject.featured ?? true,
        visibility: editingProject.visibility || "Published",
        createdAt: new Date().toISOString().split("T")[0]!,
      };
      updateProjects([newProj, ...projects], `Created Project: ${newProj.title}`);
    } else if (editingProject.id) {
      const updated = projects.map((p) =>
        p.id === editingProject.id ? ({ ...p, ...editingProject } as ProjectItem) : p,
      );
      updateProjects(updated, `Updated Project: ${editingProject.title}`);
    }
  ...
  const handleImageFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setEditingProject((prev) => ({ ...prev, image: reader.result as string }));
      };
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-xl text-foreground font-sans overflow-hidden">
      ...
      <header className="relative z-10 flex h-16 shrink-0 items-center justify-between border-b border-border bg-card/80 px-6 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <VisezWorksIcon className="size-8 text-foreground" />
          <div>
