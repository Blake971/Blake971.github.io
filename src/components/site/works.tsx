      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-10">
        ...
        <div className="mt-12 overflow-x-auto pb-3 pt-1 scrollbar-none">
          <div
            ...
            onKeyDown={(e) => {
              const allCategories = ["All", "Featured", ...PROJECT_CATEGORIES];
        ...
        {activeModalProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/85 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-card p-6 shadow-2xl relative space-y-5"
            >
              <button
                onClick={() => setActiveModalProject(null)}
                className="absolute right-4 top-4 z-10 rounded-full bg-background/80 p-1.5 text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors"
              >
                <X className="size-5" />
              </button>
              ...
              <div className="relative overflow-hidden rounded-xl border border-border bg-black aspect-video">
                ...
                {activeModalProject.category === "Video / Editing" && (
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                    <a
                      href={activeModalProject.videoUrl || activeModalProject.link || "#"}
                      target="_blank"
                      rel="noreferrer"
                      className="grid size-16 place-items-center rounded-full bg-primary text-primary-foreground shadow-lg hover:scale-110 transition-transform"
                    >
              ...
              <div className="flex items-center justify-between border-t border-border pt-4">
                ...
                {activeModalProject.link && (
                  ...
                  >
                    Visit Live Project <ExternalLink className="size-3.5" />
                  </a>
                )}
              </div>
            </motion.div>
      ...
      >
        ...
        {isVideo ? (
          <span className="absolute inset-0 grid place-items-center bg-black/20 group-hover:bg-black/10 transition-colors">
            <span className="grid size-12 place-items-center rounded-full bg-card/90 text-foreground shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:bg-primary group-hover:text-primary-foreground">
              <Play className="size-5 translate-x-[1px]" />
            </span>
          </span>
        ) : (
          <span className="absolute bottom-3 right-3 grid size-8 place-items-center border border-border bg-card/90 text-foreground opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:text-primary rounded">
